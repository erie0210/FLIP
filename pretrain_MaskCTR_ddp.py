...
from utils import AvgMeter, get_lr, MetricLogger, SmoothedValue
...
def make_train_valid_dfs(struct_data_path, text_data_path, seed, data_source):
    ...
    train_text, test_text = text_data.iloc[:int(len(text_data) * 0.9)].copy(), \
                             text_data.iloc[int(len(text_data) * 0.9):].copy()
...
def process_struct_data(data_source, train, test, data):
    ...
    if data_source == 'movielens':
        ...
        sparse_features = ['User ID', 'Book ID', 'Book title', 'Book genres', 'Average rating', 'Number of book reviews',
                           'Author ID', 'Author name', 'Number of pages', 'eBook flag', 'Format', 'Publisher',
                           'Publication year', 'Work ID', 'Media type']
...
if __name__ == '__main__':
    ...
    for epoch in range(cfg.epochs):
        ...
        torch.save(model, restore_path + "best.pt")
        torch.save(model.rec_encoder, restore_path + "rec_best.pt")
        torch.save(model.text_encoder, restore_path + "text_best.pt")
        torch.save(model.rec_projection, restore_path + "rec_projection.pt")
        torch.save(model.text_projection, restore_path + "text_projection.pt")
